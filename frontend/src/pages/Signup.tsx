    ...
    <div className="flex h-screen w-screen items-center justify-center bg-zinc-950 overflow-hidden relative selection:bg-blue-500/30 text-white">
      ...
      <div className="w-full max-w-md p-8 space-y-6 glass-card relative z-10 border-white/10 shadow-2xl">
        <div className="text-center">
          ...
          <h1 className="text-4xl font-black tracking-tight gradient-text">Create Account</h1>
          <p className="text-zinc-500 mt-2 font-medium">Join the next generation of student finance</p>
        </div>
        
        {error && <div className="p-3 bg-red-100/80 dark:bg-red-900/30 border border-red-400/50 text-red-700 dark:text-red-400 rounded-xl text-sm backdrop-blur-sm transition-all animate-in fade-in slide-in-from-top-1">{error}</div>}

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 ml-1">Name</label>
            <input
              type="text"
              value={name}
        ...
        <p className="text-center text-sm text-zinc-600 dark:text-zinc-400">
          Already have an account? <Link to="/login" className="text-blue-600 dark:text-blue-400 hover:underline font-semibold">Log in</Link>
        </p>
      </div>
    </div>
  );
